...
class _MyhomeState extends State<Myhome> {
  ...
  void initState() {
    ...
    _isSearching = new Searchpostmodel(
    ...
    if (_controller.offset >= _controller.position.maxScrollExtent &&
      ...
      if (_currentPage <= totalPage) {
        if (_isSearching.category != null) {
          _pagesearchObjModel = new Pagesearchobjmodel(
              _isSearching,
              null,
              _isSearching.category,
              new Pageoptmodel(_currentPage, _pageSize, ['date'], [true]));
          setState(() {
            _heroTagIndex += 1;
            _heroTag = "btnSearching" + _heroTagIndex.toString();
            _future =
                fetchSearchDisplayPosts(context, _pagesearchObjModel.toJson());
          });
        } else {
          setState(() {
            _future = fetchDisplayPosts(context, _pageObjModel.toJson());
          ...
          onPressed: () async {
            ...
            if (_isSearching.category != null) {
              ...
              setState(() {
                _heroTagIndex += 1;
                _heroTag = "btnSearching" + _heroTagIndex.toString();
                _future = fetchSearchDisplayPosts(
                    context, _pagesearchObjModel.toJson());
              });
            }
          },
